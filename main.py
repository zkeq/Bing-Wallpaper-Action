...
path = os.path.join(os.path.dirname(__file__), 'data', 'daily_log', "{}.json".format(get_now_time()).replace(" ", "_").replace(":", "-"))
with open(path, "w", encoding="utf-8") as _f_:
    json.dump(data, _f_, ensure_ascii=False, indent=4)


def read_update_json():
    path = os.path.join(os.path.dirname(__file__), 'data', 'update.json')
...
with open(os.path.join(os.path.dirname(__file__), 'data', 'temp.json'), "w", encoding="utf-8") as _f:
...
print("[{}] 开始更新 all.json".format(get_now_time()))
print("[{}] 更新前 all.json 数量：{}".format(get_now_time(), len(all_data["data"])))

before_all_list = all_data["data"]
write_list.extend(before_all_list)

print("[{}] 更新后 all.json 数量：{}".format(get_now_time(), len(write_list)))
...
print("[{}] 更新 all.json 成功".format(get_now_time()))
...
print("[{}] 更新 update.json 成功".format(get_now_time()))
